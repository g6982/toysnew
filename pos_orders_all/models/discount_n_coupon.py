...
class PosOrder(models.Model):
	...
	def _onchange_amount_all(self):
		for order in self:
			...
			order.amount_tax = currency.round(sum(order._amount_line_tax(line, order.fiscal_position_id) for line in order.lines))
	...
	def _process_order(self, order, draft, existing_order):
		...
		#Todo: Nuevo >> Crear pago para luego conciliar
		self._validate_payments_refunds_used(order,pos_order) #Agregado para el tema de pagos usados
		if not draft:
			try:
	...
	def _validate_payments_refunds_used(self, order,pos_order):
		if order and pos_order:
			for payments in order['statement_ids']:
				if 'payments_refund' in payments[2]:
					monto_pagado_en_orden = round(pos_order.amount_paid,2)
					for pay_refund in payments[2]['payments_refund']:
						pos_payment_refund = self.env['pos.payment'].sudo().browse(int(pay_refund['id']))
						# if pos_payment_refund.is_used == True:
						# 	raise (_('El pago ya fue usado en la orden con referencia:  %s' % (pos_payment_refund.order_used.pos_reference)))
						amount_paid = pos_payment_refund.amount
						monto_pagado_en_orden = abs(monto_pagado_en_orden) - abs(amount_paid)
						if monto_pagado_en_orden > 0.0:
							pos_payment_refund.amount_used = 0.0
							pos_payment_refund.sudo().write({'is_used': True})
						else:
							pos_payment_refund.amount_used = abs(amount_paid) - abs(monto_pagado_en_orden)
							pos_payment_refund.sudo().write({'is_partial_used': True})
						pos_order.payments_refund_used_ids += pos_payment_refund
						pos_payment_refund.orders_used += pos_order
