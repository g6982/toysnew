...
odoo.define('pos_cash_control.pos_cash_control', function (require) {
    ...
    class CashControlPopupWidget extends AbstractAwaitablePopup {
         get_accounts_in(){
            ...
            return rpc.query({
                ...
                method: 'get_accounts_in',
                args: [self.env.pos.config.account_cash_in],
            }).then(function (result) {
                console.log(result);
                self.props.cuentas_in = result;
        ...
        click_cash_in(event){
           ...
           rpc.query({
                ...
                console.log(result);
                self.showPopup('CashInPopupWidget', {
                    'title': self.env._t('Cash IN'),
                    'accounts_in': result,
                });
            }).catch(function (error) {
                console.log("---fail---",error)
        ...
        click_cash_out(event){
            ...
            rpc.query({
                model: 'account.account',
                method: 'get_accounts_in',
                args: [self.env.pos.config.account_cash_out],
            }).then(function (result) {
                console.log(result);
                self.showPopup('CashOutPopupWidget', {
                    'title': self.env._t('Cash OUT'),
                    'accounts_out': result,
                });
            }).catch(function (error) {
                console.log("---fail---",error)
    ...
    class CashInPopupWidget extends AbstractAwaitablePopup {
        ...
        click_confirm(event){
            ...
            if($("#select_account_in").val()==""){
                alert('Debe seleccionar alguna cuenta. ')
                return
            }
            ...
            var cash_in_details = {
                ...
                'pos_session_id' : self.env.pos.pos_session.id,
                'type': 'cash_in',
                'account_id': $("#select_account_in").val()
            }
            rpc.query({
                model: 'pos.session',
    ...
    class CashOutPopupWidget extends AbstractAwaitablePopup {
        ...
        click_confirm(event){
            ...
            var cash_in_details = {
                ...
                'pos_session_id' : self.env.pos.pos_session.id,
                'type': 'cash_out',
                'account_id': $("#select_account_out").val()
            }
            rpc.query({
                model: 'pos.session',
                method: 'create_check_in_out',
    ...
    class PaymentsPopupWidget extends AbstractAwaitablePopup {
        ...
        confirm_change_payment(event){
            var self = this;
            var payment_id = event;
            var select = $("#"+ payment_id+ "_ppm");
            var select_val = select[0].value
            ...
            rpc.query({
                model: 'pos.payment',
                method: 'update_pos_payment_from_pos',
                args: [payment_id, parseInt(select_val)],
            }).then(function (result) {
                ...
                self.showPopup('ErrorPopup', {
                    ...
                    body: self.env._t('No se pudo hacer el cambio.'),
                });
            });
