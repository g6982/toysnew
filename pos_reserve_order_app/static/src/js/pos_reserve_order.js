odoo.define('pos_reserve_order_app.pos_reserve_order', function(require) {
	...
	const PosReservePaymentScreen = PaymentScreen =>
		class extends PaymentScreen {
			...
			reserve_order() {
				...
				let today = new Date();
                let tree_getMonth = new Date(today.getFullYear(), today.getMonth() + 3, today.getDate());
                var mom = moment();

                var dia = tree_getMonth.getDate();
                if (dia<10){
                    dia = '0'+String(dia);
                }
                var mes = tree_getMonth.getMonth();
                if (mes<10){
                    mes = '0'+String(mes);
                }
                var anio = tree_getMonth.getFullYear();
                var fecha = String(dia)+'-'+String(mes)+'-'+String(anio);
				if($('.reserve').hasClass('highlight') ){
                    var aho = mom.format('YYYY-MM-DD');
		...
		create_reserve_order() {
			...
			if(!entered_date){
				...
				if(d1 > d2){
					...
					order.saldo = total - order.get_total_paid();
	...
	class SeeReservedOrdersScreenWidget extends PosComponent {
		...
		keyup_order_search(event){
			var self = this;
            var orders = self.env.pos.get('reserved_orders_list');
            this.render_list_orders(orders, event.target.value);
        }

        refresh(){
			$('.input-search-orders-reserved').val('');
			//this.state.query = '';
...
//		}
		...
		render_list_orders(orders, search_input){
			...
			var ordersline_html = "";
			if(orders != undefined){
				for(var i = 0, len = Math.min(orders.length,8000); i < len; i++){
					var order = orders[i];
					if(order.state == 'reserved'){
                        current_date = field_utils.format.datetime(moment(order.date_order), {type: 'datetime'});
                        ordersline_html += QWeb.render('OrdersLine',{widget: this, order:orders[i], selected_partner_id: orders[i].partner_id[0],current_date:current_date});
    ...
    //					ordersline = ordersline.childNodes[1];
				...
				//TODO: Limpiar el contenido antes de actualizar
				content.empty();
				content.append(ordersline_html);
			}
		}
	...
	class PayReserveOrderPopupWidget extends PosComponent {
		...
		generate_report(order_id){
            return this.env.pos.do_action('pos_reserve_order_app.pos_order_paid_report',{additional_context:{
                       active_ids:[order_id],
            }});
        }
			...
			if(pay_amount)
			{
				...
				rpc.query({
					...
					args: [order.id,parseInt(select_journal_id),parseFloat(pay_amount),parseInt(cash_jrnl_id),parseInt(session_id)],
					     ...
					     swal("Bien !", "Has hecho de un pago de ₡ "  + pay_amount , "success").then((value) => {
					        $('#icon_refresh').click();
                             return self.env.pos.do_action('pos_reserve_order_app.pos_order_paid_report',{additional_context:{
                               active_ids:[order.id],
                             }});
				...
				alert("Ingrese la cantidad para completar el pedido.")
			}
			this.trigger('close-popup')
		}
	}
