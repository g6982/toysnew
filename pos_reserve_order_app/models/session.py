...
class PosSessionInherit(models.Model):
	...
	def _accumulate_amounts(self, data):
		...
		payments_reserved = self.env['pos.payment'].search([('session_id','=',self.id),('pos_order_id','in',self.order_reserved_ids.ids)])
		# if all_payments_reserved:
		# 	payments_reserved = all_payments_reserved.order_reserved_ids.filtered(lambda o: o.session_id.id == self.id)

		if payments_reserved:
			for pay in payments_reserved:
				monto, fecha = pay.amount, pay.payment_date
				if pay.payment_method_id.split_transactions:
					if pay.payment_method_id.is_cash_count:
						split_receivables_cash[pay] = self._update_amounts(split_receivables_cash[pay],
																			   {'amount': monto}, fecha)
					else:
						split_receivables[pay] = self._update_amounts(split_receivables[pay],
																		  {'amount': monto}, fecha)
				else:
					#TODO RESERVE-JHONNY: Cambios para captura de montos
					if pay.session_id.id == self.id:
						key = pay.payment_method_id
						if pay.payment_method_id.is_cash_count:
							combine_receivables_cash[key] = self._update_amounts(combine_receivables_cash[key],
																				 {'amount': monto}, fecha)
						else:
							combine_receivables[key] = self._update_amounts(combine_receivables[key], {'amount': monto},
																		fecha)
		...
		for order in self.order_ids:
			...
			for payment in order.payment_ids:
				...
				if payment.payment_method_id.split_transactions:
					...
					#TODO RESERVE-JHONNY: Cambios para captura de montos
					if payment.session_id.id == self.id:
						key = payment.payment_method_id
						if payment.payment_method_id.is_cash_count:
							combine_receivables_cash[key] = self._update_amounts(combine_receivables_cash[key],
																				 {'amount': amount}, date)
