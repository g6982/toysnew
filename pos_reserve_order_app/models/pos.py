...
class POSOrderInherit(models.Model):
	...
	#TODO: MEJORAS PARA PAGO DE RESERVE POS
	def pay_reserved_amount(self,select_journal_id,pay_amount,cash_jrnl_id,session_id):
		...
		if self.state == 'reserved' and self.session_id.id != session_id:
			session.order_reserved_ids = session.order_reserved_ids + self
		...
		if pay_amount == self.amount_due :
			...
			#Comentado el 03-05-2022
			# for line in self.lines:
			# 	if line.price_unit > 0:
			# 		self.update_stock_qty_reserve(line)
			self._create_order_picking()
		else:
			self.write({
			'amount_paid' : sum(payment.amount for payment in self.payment_ids),
	...
	def update_stock_qty_reserve(self, line):
		if line.product_id.type == 'product':
			stock_quant = self.env['stock.quant'].sudo()
			reserved_loc = line.order_id.config_id.reservation_location.id
			reserved_loc_stock = stock_quant.search(
				[('product_id', '=', line.product_id.id), ('location_id', '=', reserved_loc)])
			reserved_loc_qty = 0
			if reserved_loc_stock and self.is_reserved and self.state == 'paid':
				for reserve in reserved_loc_stock:
					reserved_loc_qty = reserve.quantity
					reserved_loc_stock.write({
						'quantity': reserved_loc_qty - line.qty
