...
class ReplacementImportWizard(models.TransientModel):
    _name = 'replacement.import.wizard'
    _description = 'Importación de Reposición'
    ...
    def _process_sheet(self,sheet):
        ...
        for i in range(desde, hasta):
            ...
            name_location = sheet.cell(i,1).value
            name_route = sheet.cell(i,2).value
            quantity_min = sheet.cell(i,3).value
            quantity_max = sheet.cell(i,4).value
            qty_to_order = sheet.cell(i,5).value
            ...
            route = self.env['stock.location.route'].sudo().search([('name', '=ilike', name_route + '%')])

            if not route:
                orders_errors.append('Linea:' + str(pos) + ' - la ruta : ' + str(name_route) + ' no fue encontrada.')
            elif not location:
        ...
        return {
            ...
            u'view_mode': u'form',
            u'target': u'new',
            u'res_model': u'replacement.import.wizard',
            u'res_id': self.id
        }

    def download_file_xlx(self):
        self.ensure_one()
        url = f'/l10n_cr_toy_extends/static/xlsx/CARGA_REPOSICION.xlsx'
        return {
            'name': _("Excel"),
    ...
    def link_action_orderpoint_replenish(self):
