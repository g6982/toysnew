...
class PosCouponApplyCode(models.TransientModel):
    ...
    def process_coupon(self):
        ...
        """
        pos_order = self.env['pos.order'].browse(self.env.context.get('active_id'))
        error_status = self.apply_coupon(pos_order, self.coupon_code)
        ...
        if program:
            ...
            if not error_status:
                if program.promo_applicability == 'on_next_order':
                    ...
                    order._create_reward_line_pos(program)
            ...
            if coupon:
                ...
                if not error_status:
                    order._create_reward_line_pos(coupon.program_id)
