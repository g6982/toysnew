...
class Http(models.AbstractModel):
    ...
    def session_info(self):
        ...
        # user = request.env.user
        # if self.env.user.has_group('base.group_user'):
        #     result['user_branches'] = {'current_branch': (user.branch_id.id, user.branch_id.name), 'allowed_branch': [(comp.id, comp.name) for comp in user.branch_ids]},
        #     result['display_switch_branch_menu'] = user.has_group('branch.group_multi_branch') and len(user.branch_ids) > 1,
        #     result['user_branches'] = user.branch_id.ids
        #
        # return result
        #
        # user = request.env.user
